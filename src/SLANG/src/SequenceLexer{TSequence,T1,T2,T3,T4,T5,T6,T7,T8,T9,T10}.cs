namespace SLANG
{
    ...
    public abstract class SequenceLexer<TSequence, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10> : Lexer<TSequence>
        ...
        where T10 : Element
    {
        /// <summary>Initializes a new instance of the <see cref="SequenceLexer{TSequence,T1,T2,T3,T4,T5,T6,T7,T8,T9,T10}"/> class for an unnamed element.</summary>
        ...
        public override bool TryRead(ITextScanner scanner, out TSequence element)
        {
            ...
            T10 element10;
            if (!this.TryRead10(scanner, out element10))
            {
        ...
        {
            return (TSequence)Activator.CreateInstance(typeof(TSequence), element1, element2, element3, element4, element5, element6, element7, element8, element9, element10, context);
        }
