namespace SLANG
{
    ...
    public abstract class SequenceLexer<TSequence, T1, T2> : Lexer<TSequence>
        ...
        where T2 : Element
    {
        /// <summary>Initializes a new instance of the <see cref="SequenceLexer{TSequence,T1,T2}"/> class for an unnamed element.</summary>
        ...
        protected virtual TSequence CreateInstance(T1 element1, T2 element2, ITextContext context)
        {
            return (TSequence)Activator.CreateInstance(typeof(TSequence), element1, element2, context);
        }
